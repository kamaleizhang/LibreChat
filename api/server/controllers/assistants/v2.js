...
const addResourceFileId = async ({req, openai, assistant_id, tool_resource, file_id}) => {
    ...
    if (tool_resource === 'file_search') {
        ...
        const myVectorStoreFile = await openai.beta.vectorStores.files.create(vectorStoreId, {file_id}).catch(error => {
            logger.error('error adding file to vector store', error);
            throw error
        });
        logger.debug('added file to vector store', myVectorStoreFile);
        //add file_id to assistants metadata 用于回显
        const assistantMetadata = assistant.metadata;
        let fileStr = assistantMetadata.files || ""
        let files = fileStr.split(",")
        files.push(file_id)
        fileStr = files.join(",")
        assistantMetadata.files = fileStr
        await openai.beta.assistants.update(assistant_id, {metadata: assistantMetadata});
    } else if (tool_resource === 'code_interpreter') {
