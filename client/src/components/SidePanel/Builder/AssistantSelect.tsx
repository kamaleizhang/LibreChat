		...
		select: (res) =>
			res.data.map((_assistant) => {
				...
				const assistant: TAssistantOption = {
					...
					value: _assistant.id,
					files: _assistant.metadata?.files ? ([] as Array<[string, ExtendedFile]>) : undefined,
					code_files: _assistant.tool_resources?.code_interpreter?.file_ids ? ([] as Array<[string, ExtendedFile]>) : undefined,
				};
				...
				if (assistant.files && _assistant.metadata?.files) {
					let fileStr = _assistant.metadata?.files || ""
					// @ts-ignore
					let file_ids = fileStr.split(",")
					file_ids.forEach((file_id: string) => {
						if (file_id === "") {
							return
						}
						handleFile(file_id, assistant.files)
					});
				}
